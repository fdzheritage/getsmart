...
if (isset($_GET['logout'])) {
  setcookie('user', '', time() - 1); // expiration time in the past
  setcookie('code', '', time() - 1);     // expiration time in the past
} else {
  ...
  if (
    ...
    if ($result['status']) {
      $site_user = $result['user']; // Database row
